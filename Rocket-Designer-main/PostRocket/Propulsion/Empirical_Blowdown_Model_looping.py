...
for N in np.floor(np.logspace(1,3,20)):
    for M in np.floor(np.logspace(1,3,20)):
        ...
        with open("t_"+str(N)+"_x_"+str(M)+"Rdot.csv",'w',newline='') as file:
            writer = csv.writer(file)
            writer.writerows(rdot)

        with open("t_"+str(N)+"_x_"+str(M)+"MDOT.csv",'w',newline='') as file:
            writer = csv.writer(file)
            writer.writerows(MDOT)


        with open("t_"+str(N)+"_x_"+str(M)+"ThrustData.csv",'w',newline='') as file:
            writer = csv.writer(file)
